...
export const plugin = new PanelPlugin<JiraOptions>(JiraPanel).setPanelOptions((builder) => builder
  ...
  .addTextInput({ path: 'jiraBaseUrl', name: 'Jira base URL', description: 'HTTPS URL, including any Jira context path. Used only for ticket links, never for fetching data.', defaultValue: defaults.jiraBaseUrl })
